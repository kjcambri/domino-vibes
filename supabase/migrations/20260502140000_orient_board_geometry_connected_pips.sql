-- Sprint 9 patch: orient server-saved domino geometry so the connected pip
-- faces the endpoint. Asset convention: domino-low-high.png is normalized
-- low-on-top and high-on-bottom, so tile.left is the unrotated top half and
-- tile.right is the unrotated bottom half.
...
declare
  ...
  v_rotation integer;
begin
  -- For a normal horizontal start tile, rotate 270deg so tile.left faces the
  -- saved left endpoint and tile.right faces the saved right endpoint.
